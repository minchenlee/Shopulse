...
function Direction(props){
  const { title, contentList, onClick } = props;
  return(
    <div className='flex flex-col justify-between h-full items-start'>
      <h3 className='font-russoOne text-xl mb-3'>{title}</h3>
      <ul className='font-nunito ps-6 pl-6 list-disc list-outside space-y-[6px]'>
      ...
      <button
      onClick={onClick} 
      className='mt-auto text-grey font-russoOne text-xl hover:text-primary duration-300 ease-in-out'>
        Previous
      </button>
    </div>
  )
...
function NextButton(props){
  ...
  return(
    <button 
    onClick={onClick}
    className='w-full h-full px-6 pt-6 pb-4 flex flex-col justify-between items-end group'>
      <h3 className='font-russoOne text-white text-xl text-end'>{title}</h3>
      <h3 className='text-white font-russoOne text-xl group-hover:text-grey duration-300 ease-in-out'>
        Next
      </h3>
    </button>
  )
